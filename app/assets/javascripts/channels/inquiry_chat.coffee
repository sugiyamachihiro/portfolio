  ...
  App.inquiry_chat = App.cable.subscriptions.create "InquiryChatChannel",
    ...
    received: (data) ->
      if data.inquiry_message.from_id == $('input:hidden[name="from_id"]').val()
        currentUserMsg = '<div class="message" data-session=""><div class="message_right" style="margin: 30px 0 30px 0;padding: 10px;width: 100%;"><span class="" id="message_content" style="background-color: #ffe4e1;font-size: 30px;">' + data.inquiry_message.content.replace(/¥"/g,"") + '</span></div></div>'
        $('#inquiry_messages').append currentUserMsg
      else
         otherUserMsg = '<div class="message" data-session=""><div class="message_left" style="margin: 30px 0 30px 0;padding: 10px;width: 100%;"><span class="" id="message_content" style="background-color: #efefef;font-size: 30px;">' + data.inquiry_message.content.replace(/¥"/g,"") + '</span></div></div>'
